...
//public class Main {
//
//    static String [][] mbookList= new String[3][7];
//    // 카트아이템 참조값 배열임.
//    static CartItem[] mCart = new CartItem[3];//참조값 저장할 공간만 10개 만듬
...
//        // 이렇게 하면 유저에 대하여 네임과 폰이 입력받은 객체가 생성됨.
//
//
...
//    }
//
//    public static void menuGuestInfo(Person user){// 퍼슨형 객체 참조변수로 표시
...
//            for(int j=0;j<mbookList[0].length;j++) {
//                System.out.print(mbookList[i][j]+"|");
//            }
//            System.out.print("\n");
//        }
//        //
...
//
//            for (int i = 0; i < mbookList.length; i++) {
//                if (mbookList[index][0].equals(ID)) {
//                    // 반대로도 가능.
//                    flag = !flag;
//                    index=i;
...
//        // 담겨있는 것인지 확인, 카트 정보도 넘겨야 함.
//        for(int i=0;i<mCartItemCount;i++){// this는 메모리에 현재 내 위치. 디스는 정적인 멤버에게는 안붙임
//            if(bookId.equals(mCart[i].getBook()[0])) {// 스트링 배열이 리턴됨
